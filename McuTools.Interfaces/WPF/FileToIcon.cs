...
namespace McuTools.Interfaces.WPF
{
    ...
    public class FileToIconConverter : IMultiValueConverter
    {
        ...
        internal static Icon GetFileIcon(string fileName, IconSize size)
        {
            ...
            uint flags = NativeConsts.SHGFI_SYSICONINDEX;
            if (fileName.IndexOf(":") == -1)
                flags = flags | NativeConsts.SHGFI_USEFILEATTRIBUTES;
            if (size == IconSize.small)
                flags = flags | NativeConsts.SHGFI_ICON | NativeConsts.SHGFI_SMALLICON;
            else flags = flags | NativeConsts.SHGFI_ICON;
